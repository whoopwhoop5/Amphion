...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run FasterSVC over a playlist manifest (offline or streaming sim)."
    )
    ...
    for pair_idx, pair in enumerate(pairs):
        ...
        if not args.stream:
            ...
            if frame_size > 0 and (hop_in % frame_size) != 0:
                raise ValueError(
                    f"hop must be a multiple of frame_size={frame_size} (got hop_in={hop_in})"
                )
            ...
            for start in range(0, len(src_24k), hop_in):
                ...
                if not voiced:
                    ...
                    out_window = _infer_fastersvc(
                        ...
                        src_wav_24k=window,
                        tgt_feat=tgt_feat,
                        seed=int(args.seed) + int(pair_idx) * 100000 + int(window_count),
                        device=device,
                        k=int(args.k),
                        alpha=float(args.alpha),
                        pitch_shift=float(args.pitch_shift),
        ...
        out_meta.write_text(json.dumps({"config": cfg, "stats": stats}, indent=2), encoding="utf-8")

        print(f"[fastersvc_playlist] {cid}", flush=True)
