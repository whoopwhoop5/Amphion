...
def _infer_freevc(
    *,
    ...
    # Inline a tiny subset of `freevc_convert.py` so this runner is standalone.
    import librosa

    ref_wav_16k = np.asarray(ref_wav_16k, dtype=np.float32).reshape(-1)
    if float(ref_trim_db) > 0 and len(ref_wav_16k) > 0:
        ref_wav_16k, _ = librosa.effects.trim(ref_wav_16k, top_db=float(ref_trim_db))
    ...
    src_t = torch.from_numpy(np.asarray(src_wav_16k, dtype=np.float32).reshape(-1)).unsqueeze(0).to(device)
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run FreeVC over a playlist manifest (offline or streaming sim).")
    parser.add_argument("--manifest", type=str, required=True, help="Playlist manifest.json (see build_fleurs_playlist).")
    parser.add_argument("--out_dir", type=str, required=True, help="Output directory for wavs/meta (ignored by git).")

    parser.add_argument("--freevc_dir", type=str, required=True, help="Path to FreeVC repo checkout.")
    parser.add_argument("--variant", type=str, default="freevc-24", choices=["freevc", "freevc-s", "freevc-24"])
    ...
    ckpt_name = {"freevc": "freevc.pth", "freevc-s": "freevc-s.pth", "freevc-24": "freevc-24.pth"}[variant]
    cfg_name = {"freevc": "freevc.json", "freevc-s": "freevc-s.json", "freevc-24": "freevc-24.json"}[variant]
    ...
    print(f"[freevc_playlist] Loading {variant} from {cfg_path} + {ckpt_path}", flush=True)
    ...
    print(f"[freevc_playlist] Wrote: {out_root}", flush=True)
    return 0
