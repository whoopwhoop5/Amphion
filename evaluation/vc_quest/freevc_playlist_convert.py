...
def _infer_freevc(
    *,
    ...
    src_t = (
        torch.from_numpy(np.asarray(src_wav_16k, dtype=np.float32).reshape(-1))
        .unsqueeze(0)
        .to(device)
    )
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run FreeVC over a playlist manifest (offline or streaming sim)."
    )
    parser.add_argument(
        "--manifest",
        type=str,
    ...
    parser.add_argument(
        "--vad_mode", type=str, default="webrtc", choices=["rms", "webrtc", "off"]
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=18.0,
        help="Maximum gain boost applied by match_src_rms.",
    )
    parser.add_argument(
        "--gain_smoothing",
        type=float,
    ...
    print(
        f"[freevc_playlist] Loading {variant} from {cfg_path} + {ckpt_path}", flush=True
    )
