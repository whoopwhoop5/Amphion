...
def _infer_mnpsvc(
    *,
    ...
    if float(f0_min) > 0:
        f0[f0 < float(f0_min)] = float(f0_min)
    if float(f0_max) > 0:
        f0[f0 > float(f0_max)] = float(f0_max)
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="MNP-SVC one-shot VC runner (offline or streaming simulation).")
