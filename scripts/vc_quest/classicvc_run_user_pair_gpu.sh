...
# Runs ClassicVC/MMCXLI for the user pair (offline + streaming sim),
...
RUN_DIR="runs/vc_quest/classicvc/user_pair"
mkdir -p "${RUN_DIR}"
...
echo "[classicvc_run] mmcxli_dir=${MMCXLI_DIR} model_device=${MODEL_DEVICE} ref_max_sec=${REF_MAX_SEC}"
echo "[classicvc_run] Streaming: window=${STREAM_WINDOW_MS}ms hop=${STREAM_HOP_MS}ms emit_align=${STREAM_EMIT_ALIGN}"
