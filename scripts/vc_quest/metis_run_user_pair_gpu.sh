...
# Runs Metis VC for the user pair (offline + streaming sim),
...
RUN_DIR="runs/vc_quest/metis/user_pair"
mkdir -p "${RUN_DIR}"
...
VARIANT="${VARIANT:-metis_vc}"
SEED="${SEED:-0}"
N_TIMESTEPS="${N_TIMESTEPS:-20}"
...
echo "[metis_run] variant=${VARIANT} n_timesteps=${N_TIMESTEPS} cfg=${CFG_SCALE} ref_max_sec=${REF_MAX_SEC}"
echo "[metis_run] Streaming: window=${STREAM_WINDOW_MS}ms hop=${STREAM_HOP_MS}ms emit_align=${STREAM_EMIT_ALIGN}"

python -m evaluation.vc_quest.metis_convert \
  --variant "${VARIANT}" \
  --device cuda:0 \
  --seed "${SEED}" \
...
echo "[metis_run] Wrote artifacts to ${RUN_DIR}"
