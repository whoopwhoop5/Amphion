...
# Runs MNP-SVC over the deterministic French playlist, then scores the outputs.
...
RUN_NAME="${RUN_NAME:-mnpsvc_w800_h400_end}"
...
# 2) Convert in the MNP-SVC conda env.
source "${CONDA_SH}"
conda activate "${ENV_NAME}"

python -m evaluation.vc_quest.mnpsvc_playlist_convert \
  --manifest "${MANIFEST}" \
  --out_dir "${RUN_DIR}" \
...
echo "[mnpsvc_run_fleurs_fr_playlist] Wrote ${RUN_DIR}/summary.json"
