...
# Runs Metis VC over the deterministic French playlist, then scores the outputs.
...
RUN_NAME="${RUN_NAME:-metis_vc_w800_h400}"
...
# 2) Convert in the Metis conda env.
source "${CONDA_SH}"
conda activate "${ENV_NAME}"

export HF_HOME="${HOME}/.hf_home"

python -m evaluation.vc_quest.metis_playlist_convert \
  --manifest "${MANIFEST}" \
  --out_dir "${RUN_DIR}" \
  --variant "${VARIANT}" \
  --device cuda:0 \
...
echo "[metis_run_fleurs_fr_playlist] Wrote ${RUN_DIR}/summary.json"
